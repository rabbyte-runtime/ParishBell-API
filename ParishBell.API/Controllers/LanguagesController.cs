...
public class LanguagesController(ILanguageService languageService, IMessageCache messages, ILogger<LanguagesController> logger) : ApiControllerBase(logger)
{
    ...
    [HttpGet]
    public Task<IActionResult> GetLanguages([FromHeader(Name = "Accept-Language")] string? acceptLanguage, CancellationToken ct) =>
        ExecuteAsync(nameof(GetLanguages), async () =>
        {
